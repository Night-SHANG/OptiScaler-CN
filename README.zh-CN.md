...
`Upstream Sync` 每天执行一次，也可手动运行。它负责：

`检查官方 master/稳定 Release → 拉取精确上游 commit → 扫描 UI → 保留已审核翻译 → 生成待翻译项 → 可选增量机器翻译 → 结构检查 → 验证本地化注入锚点 → 安全提交语言资源与跟踪状态`

同步成功后会触发独立的 `Build CN` Windows 干净构建；发现新的官方稳定 Release 时还会触发 `Release CN`，重新针对该精确 tag 扫描、注入、编译和打包。只有稳定版真正发布成功后，才会记录 `last_released_upstream_tag`。
...
第一次运行 `Upstream Sync` 时，由于跟踪标记为空，它会把当前官方 `master` 视为首次同步，并把当前最新稳定版视为首次待发布版本；因此正常情况下会同时触发一次开发版 Build 和一次稳定版 Release。之后只处理新的上游变化。
...
无翻译 API 时，新文本会进入 `Localization/pending.json`，构建仍可继续；配置翻译 API 后，只会处理新增、变化或缺失的文本，不会覆盖 `reviewed` 人工翻译。
...
python tools/scan_ui.py --source _upstream --update
python tools/check_localization.py
python tools/localize_source.py --source _upstream
...
- `Localization/`：英文目录、简中资源、翻译记忆、术语表、扫描规则、待翻译列表。
